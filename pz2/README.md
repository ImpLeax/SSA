...
У цьому завданні я проаналізував теоретичну можливість використання вершини стека (Stack Pointer, SP) замість лічильника команд (Instruction Pointer, IP) під час виклику процедур та повернення з них.

### Опис завдання

Необхідно було дати обґрунтовану відповідь на запитання: чи можна в архітектурі процесора обійтися без лічильника команд (IP), використовуючи замість нього лише вершину стека?

---

### Відповідь та обґрунтування

Під час аналізу я дійшов висновку, що **обійтися без лічильника команд (IP) неможливо**. Використовувати лише вершину стека (SP) для керування потоком виконання програми не можна з кількох фундаментальних причин:

1. **Різне призначення регістрів та сегментів пам'яті:**
* Регістр **IP** (Instruction Pointer) завжди вказує на адресу *наступної машинної інструкції*, яку має виконати процесор. Ці інструкції лежать у сегменті **Text**, який захищений від запису, але дозволений для виконання (Read + Execute).
* Регістр **SP** (Stack Pointer) вказує на *останній доданий елемент даних* у сегменті **Stack** (Read + Write). Стек призначений виключно для зберігання локальних змінних, аргументів функцій та адрес повернення.


2. **Механізм виклику процедур (`call` та `ret`):**
Ці регістри працюють у тандемі, а не замінюють один одного.
* Коли я викликаю функцію (інструкція `call`), процесор бере поточне значення **IP** (адресу, куди треба повернутися) і кладе його на стек (змінюючи **SP**). Після цього в **IP** завантажується адреса нової функції.
* Під час виходу з функції (інструкція `ret`) процесор зчитує значення з вершини стека (**SP**) і записує його назад у **IP**, продовжуючи виконання програми з потрібного місця.
* *Якби IP не існувало*, процесор не зміг би відрізнити, де на стеку лежить адреса повернення, а де — звичайна локальна змінна (наприклад, число `5`).


3. **Проблеми безпеки та виконання (NX-bit):**
Якби процесор використовував стек для визначення наступної команди, він би намагався інтерпретувати звичайні дані (мої локальні змінні) як машинний код. Це неминуче призвело б до краху програми (Segmentation Fault). Крім того, у сучасних ОС сегмент стека навмисно захищений від виконання (технологія NX-bit) для захисту від хакерських атак (наприклад, переповнення буфера), тому виконувати код зі стека апаратно заборонено.

### Висновок

Регістри IP та SP можна порівняти з процесом читання книги. **IP** — це очі читача, які ковзають по рядках тексту (сегмент коду). **Стек (SP)** — це блокнот із нотатками. Читач може записати в блокнот номер сторінки, на якій зупинився (зберегти адресу повернення), щоб відволіктися на інший розділ, а потім повернутися. Але читати саму історію безпосередньо з блокнота неможливо, оскільки там лежать лише службові дані.

Ось готовий розділ для твого `README.md`, написаний від першої особи. Я включив туди твій вивід із терміналу, розбір невеликої помилки при компіляції та детальний аналіз отриманих адрес пам'яті, який доводить теоретичні особливості роботи `malloc` та `mmap`.

---

## Завдання по варіантам (Варіант 2)

У цьому завданні я дослідив різницю між функціями виділення пам'яті `malloc()` (стандартна бібліотека C) та системним викликом `mmap()` (ядро Linux). Мета роботи — практично перевірити, як операційна система та бібліотека `glibc` оптимізують виділення пам'яті для об'єктів різного розміру та в які сегменти віртуального адресного простору вони потрапляють.

### Опис експерименту

Я написав програму `zd_v2.c`, яка робить три речі:

1. Виділяє малий об'єм пам'яті (1 КБ) через `malloc()`.
2. Виділяє великий об'єм пам'яті (10 МБ) через `malloc()`.
3. Виділяє великий об'єм пам'яті (10 МБ) безпосередньо через системний виклик `mmap()`.

Після цього програма виводить адреси виділених блоків пам'яті для порівняння.
...
```bash
impleax@Impleax:~/SSA/pz2/zd_v2$ gcc -Wall zd_v2.c -o prog
impleax@Impleax:~/SSA/pz2/zd_v2$ ./prog
PID: 2180

1. malloc (1 KB) address:   0x619ebc1746b0
2. malloc (10 MB) address:  0x734c8a1ff010
3. mmap (10 MB) address:    0x734c897ff000

```

---

### 2. Аналіз результатів та адрес пам'яті

Отримані адреси яскраво демонструють внутрішню механіку роботи менеджера пам'яті в Linux:

* **Малий `malloc` (1 КБ): `0x619ebc1746b0`**
Ця адреса знаходиться відносно "низько" у віртуальному адресному просторі. Вона належить до класичного сегмента **Heap (Купа)**. Для таких невеликих запитів бібліотека `glibc` розширює межі купи за допомогою системного виклику `brk()`.
* **Великий `malloc` (10 МБ): `0x734c8a1ff010`**
Ця адреса кардинально відрізняється від попередньої (починається з `0x73...` замість `0x61...`) і знаходиться значно "вище" в пам'яті.
* **Прямий `mmap` (10 МБ): `0x734c897ff000`**
Адреса блоку, створеного прямим викликом `mmap`, знаходиться майже впритул до великого `malloc` (різниця лише в зміщенні).

---

### Висновки до завдання

1. **Дві стратегії `malloc()`:** Стандартна функція `malloc()` не завжди використовує класичну Купу (Heap). Якщо запит на виділення пам'яті перевищує певний поріг (зазвичай 128 КБ), `glibc` автоматично змінює стратегію і непомітно для програміста використовує системний виклик `mmap()` під капотом.
2. **Сегмент відображення пам'яті:** Адреси `0x734c8a1ff010` та `0x734c897ff000` належать до окремого регіону пам'яті — **Memory Mapping Segment**, який зазвичай розташовується між Купою та Стеком.
3. **Чому так відбувається?** Використання `mmap()` для великих блоків дозволяє операційній системі ефективніше керувати пам'яттю. Коли такий великий блок звільняється (через `free()`), пам'ять миттєво повертається системі за допомогою `munmap()`. Якби величезні блоки виділялися в класичній Купі, це могло б призвести до сильної фрагментації пам'яті, і повернути її ОС було б складно, якщо "вище" в Купі лежать інші зайняті дані.

## Повний код: [zd_v2.c](zd_v2/zd_v2.c)
